...
{
    return s_connected && s_write_handle != 0 && s_notify_handle != 0 && s_notify_enabled;
}
...
bool conn_write(const char *data, int len)
{
    ...
    if (rc != 0) {
        ESP_LOGE(TAG, "write to 0x%04X failed: rc=%d", s_write_handle, rc);
    } else {
        ESP_LOGI(TAG, "write %d bytes to 0x%04X", len, s_write_handle);
    }
    return (rc == 0);
}
...
static void disc_chr_next_svc(uint16_t conn_handle)
{
    ...
    if (rc != 0) {
        ...
        s_cur_svc++;
        disc_chr_next_svc(conn_handle); /* 跳过失败的服务 */
    }
}
...
static void disc_dsc_next_chr(uint16_t conn_handle)
{
    ...
    if (s_cur_chr >= s_chr_count) {
        ...
        if (!s_notify_enabled && s_notify_handle != 0) {
            /* 终极 fallback：盲写 CCCD = notify_handle + 1 */
                          ...
                          uint16_t chr_val_handle, const struct ble_gatt_dsc *dsc, void *arg)
{
    ...
    if (dsc->uuid.u.type == BLE_UUID_TYPE_16 &&
        ...
        /* 启用成功，标记已完成，但 disc_finish 由 dsc_next_chr 在循环结束时统一调用 */
...
static void disc_finish(void)
{
    ...
    /* 优先选择 0x18F0 (iCar Pro / ELM327 标准协议) 的 write/notify handle */
    ...
    /* 如果优先选到了 notify，但 CCCD 不是它的，尝试重新启用 */
